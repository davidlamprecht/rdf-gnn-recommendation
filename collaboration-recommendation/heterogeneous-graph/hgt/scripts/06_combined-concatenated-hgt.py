...
f = open('eval-soa-aa/hgt/results/06_eval-hgt-combined-concatenated.txt', 'w')
sys.stdout = f
...
class Model(torch.nn.Module):
    def __init__(self, out_channels):
        super().__init__()
        self.work_lin = torch.nn.Linear(264, out_channels)
        self.author_lin = torch.nn.Linear(132, out_channels) 
        self.source_lin = torch.nn.Linear(139, out_channels)
        self.publisher_lin = torch.nn.Linear(134, out_channels) 
        self.concept_lin = torch.nn.Linear(146, out_channels) 
        self.institution_lin = torch.nn.Linear(140, out_channels)
