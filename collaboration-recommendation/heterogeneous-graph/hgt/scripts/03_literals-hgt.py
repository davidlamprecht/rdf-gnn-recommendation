...
f = open('eval-soa-aa/hgt/results/03_eval-hgt-literals.txt', 'w')
sys.stdout = f
...
class Model(torch.nn.Module):
    def __init__(self, out_channels):
        super().__init__()
        self.work_lin = torch.nn.Linear(136, out_channels)
        self.author_lin = torch.nn.Linear(4, out_channels) 
        self.source_lin = torch.nn.Linear(11, out_channels)
        self.publisher_lin = torch.nn.Linear(6, out_channels) 
        self.concept_lin = torch.nn.Linear(18, out_channels) 
        self.institution_lin = torch.nn.Linear(12, out_channels)
