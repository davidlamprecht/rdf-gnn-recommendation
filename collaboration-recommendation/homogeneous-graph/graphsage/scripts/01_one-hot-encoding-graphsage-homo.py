...
f = open('eval-soa-aa-homogeneous/graphsage/results/01_eval-graphsage-homo-one-hot-encoding.txt', 'w')
sys.stdout = f
...
#load data
author_csv = "soa-sw-homogeneous-author/idx/nodes-nld/author_nld.csv"
author_df = pd.read_csv(author_csv, header=None)
author_df = author_df.astype(float)

num_work_author = len(author_df)
one_hot_work = torch.eye(num_work_author)
x = one_hot_work
...
author_tensor = random_projection(x, len(author_df), target_dim)


#author_lin_dec = torch.nn.Linear(128, 64)
#author_tensor = author_lin_dec(author_tensor)
